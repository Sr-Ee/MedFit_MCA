...
if(isset($_POST['submit'])){
    ...
    include 'C:\medfit1\MedFit_MCA\includes\db.php';  
    ...
    if($user_role === "student"){
       ...
       if(preg_match("/^[a-z][a-zA-Z0-9$]+(@ascs.sies.edu.in)/ix",$college_email)){
       //if(preg_match("/^[a-z][a-zA-Z0-9$]+(@gmail.com)/ix",$college_email)){

            $sql = "SELECT `COL_EMAIL` FROM `users` WHERE `COL_EMAIL` = '$college_email'";
        ...
        if(preg_match("/^[a-z][a-zA-Z0-9$]+(@sies.edu.in)/ix",$college_email)){
        //if(preg_match("/^[a-z][a-zA-Z0-9$]+(@gmail.com)/ix",$college_email)){
    ...
    if(empty($error_array)){
        ...
        $query = "INSERT INTO `users`(`FIRST_NAME`,`MIDDLE_NAME`,`LAST_NAME`,`COL_EMAIL`,`PASSWORD`,`SIGNUP_DATE`,`USER_ROLE`,`VERIFICATION_ID`,`VERIFY_STATUS`,`PHONE_NUM`,`PROFILE_PIC`) VALUES ('$fname','$mname','$lname','$college_email','$password','$signup_date','$user_role','$verification_id',0,'$phone_num','java.png')";
...
function smtp_mailer($to,$subject,$msg){
  try{
    require 'C:/xampp/htdocs/colnet/php_mailer/vendor/autoload.php';
    ...
    $mail->Password   = 'Pandurang29@1971';                               // SMTP password
